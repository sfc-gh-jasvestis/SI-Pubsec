...
CREATE STAGE IF NOT EXISTS SG_PUBSEC_DEMO.SEMANTIC_MODELS.ANALYST_STAGE
    COMMENT = 'Stage for Cortex Analyst semantic model YAML files';

-- Grant necessary privileges for Cortex Analyst
GRANT USAGE ON SCHEMA SG_PUBSEC_DEMO.SEMANTIC_MODELS TO ROLE SNOWFLAKE_INTELLIGENCE_ADMIN;
GRANT READ ON STAGE SG_PUBSEC_DEMO.SEMANTIC_MODELS.ANALYST_STAGE TO ROLE SNOWFLAKE_INTELLIGENCE_ADMIN;

-- Create semantic model for Singapore Citizen Services Analytics
CREATE OR REPLACE FILE FORMAT SG_PUBSEC_DEMO.SEMANTIC_MODELS.YAML_FORMAT
    TYPE = 'CSV'
...
CREATE OR REPLACE VIEW SG_PUBSEC_DEMO.ANALYTICS.CITIZEN_SERVICE_ANALYTICS AS
SELECT 
...
JOIN SG_PUBSEC_DEMO.SERVICES.SERVICE_INTERACTIONS si 
    ON cp.CITIZEN_ID = si.CITIZEN_ID;

-- Create aggregated view for policy impact analysis
CREATE OR REPLACE VIEW SG_PUBSEC_DEMO.ANALYTICS.POLICY_PERFORMANCE_ANALYTICS AS
SELECT 
    pi.POLICY_NAME,
...
CREATE OR REPLACE VIEW SG_PUBSEC_DEMO.ANALYTICS.SERVICE_PERFORMANCE_ANALYTICS AS
SELECT 
    sp.SERVICE_NAME,
    sp.AGENCY,
    sp.METRIC_TYPE,
...
GRANT SELECT ON SG_PUBSEC_DEMO.ANALYTICS.CITIZEN_SERVICE_ANALYTICS TO ROLE SNOWFLAKE_INTELLIGENCE_ADMIN;
GRANT SELECT ON SG_PUBSEC_DEMO.ANALYTICS.POLICY_PERFORMANCE_ANALYTICS TO ROLE SNOWFLAKE_INTELLIGENCE_ADMIN;
GRANT SELECT ON SG_PUBSEC_DEMO.ANALYTICS.SERVICE_PERFORMANCE_ANALYTICS TO ROLE SNOWFLAKE_INTELLIGENCE_ADMIN;
