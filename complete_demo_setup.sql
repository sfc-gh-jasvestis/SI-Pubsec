...
SELECT
    ...
    MONTH(si.INTERACTION_TIMESTAMP) as INTERACTION_MONTH,
    DAYOFWEEK(si.INTERACTION_TIMESTAMP) as DAY_OF_WEEK,
    HOUR(si.INTERACTION_TIMESTAMP) as HOUR_OF_DAY,
    
    -- Performance metrics
    si.DURATION_MINUTES,
    si.SATISFACTION_SCORE as INTERACTION_SATISFACTION,
    si.INTERACTION_STATUS,
    ...
    CASE 
        WHEN si.SATISFACTION_SCORE >= 4 THEN 'High'
        WHEN si.SATISFACTION_SCORE >= 3 THEN 'Medium'
        ELSE 'Low'
    END as SATISFACTION_CATEGORY,
    
    -- Aggregation-friendly columns
    1 as INTERACTION_COUNT,
    CASE WHEN si.INTERACTION_STATUS = 'Completed' THEN 1 ELSE 0 END as SUCCESS_COUNT,
    CASE WHEN si.INTERACTION_STATUS != 'Completed' THEN 1 ELSE 0 END as FAILURE_COUNT
