...
HOURLY_SERVICES AS (
    ...
    JOIN SG_PUBSEC_DEMO.CITIZEN_DATA.CITIZEN_PROFILES cp ON si.CITIZEN_ID = cp.CITIZEN_ID
...
CREATE OR REPLACE VIEW SG_PUBSEC_DEMO.ANALYTICS.WEATHER_EMERGENCY_RESPONSE AS
...
CREATE OR REPLACE VIEW SG_PUBSEC_DEMO.ANALYTICS.SEASONAL_POLICY_WEATHER_IMPACT AS
...
CREATE OR REPLACE VIEW SG_PUBSEC_DEMO.ANALYTICS.WEATHER_SERVICE_DASHBOARD AS
...
GRANT SELECT ON SG_PUBSEC_DEMO.ANALYTICS.WEATHER_SERVICE_CORRELATION TO ROLE SNOWFLAKE_INTELLIGENCE_ADMIN;
GRANT SELECT ON SG_PUBSEC_DEMO.ANALYTICS.WEATHER_EMERGENCY_RESPONSE TO ROLE SNOWFLAKE_INTELLIGENCE_ADMIN;
GRANT SELECT ON SG_PUBSEC_DEMO.ANALYTICS.SEASONAL_POLICY_WEATHER_IMPACT TO ROLE SNOWFLAKE_INTELLIGENCE_ADMIN;
GRANT SELECT ON SG_PUBSEC_DEMO.ANALYTICS.WEATHER_SERVICE_DASHBOARD TO ROLE SNOWFLAKE_INTELLIGENCE_ADMIN;
...
FROM SG_PUBSEC_DEMO.ANALYTICS.WEATHER_SERVICE_CORRELATION 
LIMIT 5;

SELECT 'Testing Weather Emergency Response View...' as TEST;
SELECT COUNT(*) as EMERGENCY_RECORDS 
FROM SG_PUBSEC_DEMO.ANALYTICS.WEATHER_EMERGENCY_RESPONSE 
LIMIT 5;

SELECT 'Testing Seasonal Policy Weather Impact View...' as TEST;
SELECT COUNT(*) as SEASONAL_RECORDS 
FROM SG_PUBSEC_DEMO.ANALYTICS.SEASONAL_POLICY_WEATHER_IMPACT 
LIMIT 5;

SELECT 'Testing Weather Service Dashboard...' as TEST;
SELECT * FROM SG_PUBSEC_DEMO.ANALYTICS.WEATHER_SERVICE_DASHBOARD;
